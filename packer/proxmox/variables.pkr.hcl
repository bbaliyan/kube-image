...
variable "rendered_capi_manifests_dir" {
  type        = string
  description = "Local directory (on the Packer build host) containing the pre-rendered capi-install.yaml — the concatenated 'clusterctl generate provider' output for CAPI core + CAPMOX only (no bootstrap/control-plane provider — cluster-autoscaler workers join via a plain Machine.spec.bootstrap.dataSecretName, no CAPRKE2) — to copy onto the template. Always set by build.sh (a fresh mktemp -d per build, rendered via clusterctl immediately before invoking packer build) — not meant to be set by hand; there is no default so a bare 'packer build' (bypassing build.sh) fails loudly instead of silently baking a stale or missing manifest."
}

variable "capi_core_version" {
  type        = string
  description = "Cluster API core version staged via 'clusterctl generate provider --core', e.g. v1.9.5. Resolved automatically by build.sh from kube-platform's platform/platform-versions/values.yaml (capiCoreVersion) — same do-not-set-in-proxmox.auto.pkrvars.hcl caveat as k8s_version applies."
}
